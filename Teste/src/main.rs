...
fn main() {
    ...
    let query_str = "FROM edges MATCH (g:Graph {name: 'graph1731975043'})-[e:Edge]->() WHERE e.properties.Congestion_level = 0.92 AND e.properties.Distance_km = 3.72 RETURN e";
    
    // Create and parse query
    let mut query = Query::new();
    match query.parse(query_str) {
        Ok(()) => {
            // Let's examine the parsed components
            println!("Parsed Query Structure:");
            println!("----------------------");
            
            // FROM clause
            if let Some(from) = &query.from_clause {
                println!("FROM: {}", from);  // Will print "edges"
            }
            
            // MATCH clause & Graph Pattern
            if let Some(graph_pattern) = &query.graph_pattern {
                println!("Graph Pattern:");
                println!("  - Graph Name: {}", graph_pattern.graph_name);  // Will print "graph_1731015699_1"
                println!("  - Edge Variable: {}", graph_pattern.edge_var); // Will print "e"
                println!("  - Edge Label: {}", graph_pattern.edge_label);  // Will print "Edge"
                println!("  - Direction: {:?}", graph_pattern.direction);  // Will print "Outgoing"
            }
                ...
                for condition in &where_clause.conditions {
                    match &condition.value {
                        Value::Number(n) => println!("  - {}.{} {} {}", 
                            "e.properties", condition.field, condition.operator, n),
                        Value::String(s) => println!("  - {}.{} {} '{}'", 
                            "e.properties", condition.field, condition.operator, s),
                        Value::Boolean(b) => println!("  - {}.{} {} {}", 
                            "e.properties", condition.field, condition.operator, b),
                    }
                }
            }
            ...
            if let Some(ret) = &query.return_clause {
                println!("RETURN: {}", ret);  // Will print "e"
            }
        }
