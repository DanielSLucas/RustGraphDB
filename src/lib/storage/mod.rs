...
impl StorageManager {
  ...
  pub fn load_graphs(&mut self) {
    ...
    if self.graphs.is_empty() {
      println!("No existing graphs found. Starting with an empty database.");
    } else {
      println!("Loaded {} graphs from storage.", self.graphs.len());
    }
  }
  ...
  pub fn save_all_graphs(&self) {
    for (_, graph) in &self.graphs {
      if let Err(e) = self.save_graph(graph) {
        eprintln!("Failed to save graph '{}': {}", graph.name(), e);
      }
    }
  }
  ...
  pub fn list_graphs(&self) -> Vec<String> {
    self.graphs.iter().map(|(graph_name, _)| graph_name.clone()).collect()
  }
}
